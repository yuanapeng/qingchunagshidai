...
void CircularStrip::Breathe(uint8_t times, StripColor low, StripColor high)
{
	std::lock_guard<std::mutex> lock(mutex_);
    esp_timer_stop(strip_timer_);
	StripColor color = low;
	for (uint8_t i = 0; i < times; i++)
	{
		while (color.red < high.red || color.green < high.green || color.blue < high.blue)
		{
			if (color.red < high.red)
			{
				color.red++;
			...
			for (int j = 0; j < max_leds_; j++)
			{
				led_strip_set_pixel(led_strip_, j, color.red, color.green, color.blue);
			}
			led_strip_refresh(led_strip_);
			vTaskDelay(20);
		}
		while (color.red > low.red || color.green > low.green || color.blue > low.blue)
		{
			if (color.red > low.red)
			{
				color.red--;
...
void CircularStrip::Scroll(uint8_t times, StripColor high, int length)
{
	std::lock_guard<std::mutex> lock(mutex_);
    esp_timer_stop(strip_timer_);
    for (int i = 0; i < max_leds_; i++)
	{
        colors_[i] = StripColor(0, 0, 0);
    }
	...
	for (uint8_t i = 0; i < times; i++)
	{
        for (int j = 0; j < max_leds_; j++)
		{
            colors_[j] = StripColor(0, 0, 0);
        }
        for (int k = 0; k < length; k++)
		{
            int m = (offset + k) % max_leds_;
            colors_[m] = high;
        }
        for (int n = 0; n < max_leds_; n++)
		{
            led_strip_set_pixel(led_strip_, n, colors_[n].red, colors_[n].green, colors_[n].blue);
        }
        led_strip_refresh(led_strip_);
        offset = (offset + 1) % max_leds_;
		vTaskDelay(50);
...
void CircularStrip::ledShow()
{
	...
	SetAllColor(StripColor(DEFAULT_BRIGHTNESS, 0, 0));
	vTaskDelay(100);

	SetAllColor(StripColor(DEFAULT_BRIGHTNESS, 16, 0));
	vTaskDelay(100);

	SetAllColor(StripColor(DEFAULT_BRIGHTNESS, DEFAULT_BRIGHTNESS, 0));
	vTaskDelay(100);

	SetAllColor(StripColor(0, DEFAULT_BRIGHTNESS, 0));
	vTaskDelay(100);

	SetAllColor(StripColor(0, DEFAULT_BRIGHTNESS, DEFAULT_BRIGHTNESS));
	vTaskDelay(100);

	SetAllColor(StripColor(0, 0, DEFAULT_BRIGHTNESS));
	vTaskDelay(100);

	SetAllColor(StripColor(20, 0, DEFAULT_BRIGHTNESS));
	vTaskDelay(100);

	StripColor low = { 0, 0, 0 };
	StripColor high = { DEFAULT_BRIGHTNESS, 0, DEFAULT_BRIGHTNESS};
	Scroll(20, high, 5);
	Breathe(3, low, high);
...
void CircularStrip::OnStateChanged() {
    ...
    switch (device_state) {
        case kDeviceStateStarting: {
            StripColor low = { 0, 0, 0 };
            StripColor high = { low_brightness_, default_brightness_, low_brightness_};
